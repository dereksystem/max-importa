...
| Arquivo | O que cobre |
|---|---|
| `test_multiloja.py` | núcleo multi-loja: detectar mais de uma empresa, `empresaFiltro` com `IF NOT EXISTS` e grafia canônica, `_resolver_empresas` (uma loja = igual a antes; seleção respeitada; memoizado) e a cola da tela (cancelar aborta; sem conexão falha alto em vez de ignorar o multi-loja) |
...
| `test_multiloja.py` | núcleo multi-loja: detectar mais de uma empresa na `config`, `registrar_filtro` (1 linha por empresa marcada, `IF NOT EXISTS`, `emfUsuId` preenchido, grafia canônica da PK) e `_resolver_empresas` (banco de uma loja = comportamento de antes; memoiza para não consultar a `config` por linha) |
| `test_layout.py` | **geometria de janela Tk real** (3 telas × 2 resoluções): o botão de ação nunca sai da janela, o mapeamento fica com a maior parte da altura, o bloco do log não estoura o textbox, e as listas de campos ficam ordenadas por seção. Cobre também a **janela de seleção de empresas** (multi-loja) renderizada de verdade: marcar/desmarcar, o "TODAS", Cancelar, e — a lição do bug reportado em uso — que os botões estão **VISÍVEIS** (`winfo_ismapped` + altura + posição), não apenas presentes na árvore de widgets. Faz **SKIP** sozinho sem display |
